/*
** Daedalus (Version 3.2) File: wutil.cpp
...
{
  HKEY hkey;
  char szExe[cchSzMax], szIco[cchSzMax], szKey[cchSzDef], *pch;
  int iKey;
  ...
  for (iKey = 0; rgszKey[iKey]; iKey++) {
    ...
    sprintf(S(szKey), ".%s", rgszKey[iKey]);
    ...
    if (RegSetValue(hkey, NULL, REG_SZ, szKey, CchSz(szKey)) != ERROR_SUCCESS)
      return fFalse;
    if (RegDeleteKey(hkey, "shell\\open\\command") == ERROR_SUCCESS) {
      // Daedalus 2.2 and before set this.
      if (RegDeleteKey(hkey, "shell\\open") == ERROR_SUCCESS)
    ...
    sprintf(S(szKey), "Daedalus.%s\\shell\\open\\command", rgszKey[iKey]);
    if (RegCreateKey(HKEY_CLASSES_ROOT, szKey, &hkey) != ERROR_SUCCESS)
      return fFalse;
    ...
    sprintf(S(szKey), "Daedalus.%s\\shell\\edit\\command", rgszKey[iKey]);
    if (RegCreateKey(HKEY_CLASSES_ROOT, szKey, &hkey) != ERROR_SUCCESS)
      return fFalse;
    ...
    sprintf(S(szKey), "Daedalus.%s\\DefaultIcon", rgszKey[iKey]);
    if (RegCreateKey(HKEY_CLASSES_ROOT, szKey, &hkey) != ERROR_SUCCESS)
      return fFalse;
...
flag FUnregisterExtensions()
{
  ...
  for (iKey = 0; rgszKey[iKey]; iKey++) {
    ...
    sprintf(S(szKey), ".%s", rgszKey[iKey]);
    if (SHDeleteKey(HKEY_CLASSES_ROOT, szKey) != ERROR_SUCCESS)
      return fFalse;
