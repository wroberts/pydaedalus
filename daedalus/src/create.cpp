/*
** Daedalus (Version 3.2) File: create.cpp
...
LSet:
    ...
    // Hunt mode: Move to a different created cell. Search row by row in
    // opposite directions for balance, and sweep up and down for balance.
LHunt:
    fHunt = fTrue;
    do {
      if (zInc) {
...
flag CMaz::CreateMazeBraid()
{
  ...
  // There might still be a pole in the lower right corner.
...
flag CMaz::CreateMazePrim2()
{
  ...
  if (!fWall) {
    x = RndSkip(1-fWall, xh-xl-1+fWall); y = RndSkip(1-fWall, yh-yl-1+fWall);
  } else {
    for (i = 0; i <= 1; i++) {
      for (x = 2; x < xh-xl; x += 2) {
...
LNext:
    ...
    if (ms.fTreeRandom) {
      ...
      // item down through the heap so the heap is valid.
...
flag CMaz::TreeGenerate(flag fWall, int xs, int ys)
{
  ...
  loop {
    d = DirFindUncreatedCore(&x, &y, fWall);
...
flag CMaz::CreateMazeForest(flag fWall)
{
  FORS *fors;
  PT ptT;
...
flag CMaz::CreateMazeBinary()
{
  int nPercent = 50 + ms.nRndBias, zInc = NAbs(ms.cRandomAdd),
    nDir = ms.nRndRun <= 0 ? -1 : ms.nRndRun >= 100, xInc, iMax,
    x, y, i, d, xT, yT;
