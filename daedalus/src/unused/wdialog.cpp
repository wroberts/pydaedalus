/*
** Daedalus (Version 3.2) File: wdialog.cpp
...
CONST char *rgszOpen[iextMax] = {"Open Bitmap", "Open Text",
  ...
  "" /* Open Picture */, "Open 3D Bitmap", "Open Script", "Open Wireframe",
  "Open Patches", "Open File"};
CONST char *rgszSave[iextMax] = {"Save Bitmap", "Save Text",
  "Save X11 Bitmap", "Save Targa Bitmap", "" /* Save Paint Bitmap */,
  "Save Picture", "Save 3D Bitmap", "" /* Save Script */, "Save Wireframe",
  "Save Patches", "" /* Save File */};
...
CONST char *rgszExt[iextMax] = {
  "bmp", "txt", "xbm", "tga", "pcx", "wmf", "d3", "ds", "dw", "dp", ""};
...
flag API DlgFile(HWND hdlg, uint msg, WORD wParam, LONG lParam)
{
  ...
  case WM_INITDIALOG:
    ...
    SetCheck(dxFs_if, ms.fInfFile);
    SetCheck(dxFs_id, ms.fInfFileDos);
  ...
  case WM_COMMAND:
    if (wParam == IDOK || wParam == IDCANCEL) {
      if (wParam == IDOK) {
        ...
        ms.fInfFile = GetCheck(dxFs_if);
        ms.fInfFileDos = GetCheck(dxFs_id);
...
#endif
    sprintf(S(sz), "Daedalus version %s for %s Windows", szVersion, szArch);
...
flag API DlgInside(HWND hdlg, uint msg, WORD wParam, LONG lParam)
{
  ...
  case WM_INITDIALOG:
    ...
    SetCheck(dxIn_si, dr.fStar);
    SetEditN(deIn_si, dr.cStar);
