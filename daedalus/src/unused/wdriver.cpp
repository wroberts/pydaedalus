/*
** Daedalus (Version 3.2) File: wdriver.cpp
  ...
  case cmdSetupExtension:
    if (!FRegisterExtensions())
      PrintSz_E("Failed to register Daedalus file extensions.\n"
        "You may need to run Daedalus as Administrator for registering to "
        "succeed.\nThis error is ignorable and all features of Daedalus will "
        "still work.");
    break;
  ...
  case cmdUnsetup:
    if (!FUnregisterExtensions())
      PrintSz_E("Failed to unregister Daedalus file extensions.\n"
        "You may need to run Daedalus as Administrator for unregistering to "
        "succeed.");
    break;
...
LRESULT WINAPI WndProc(HWND hwnd, uint wMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  byte rgbKey[256];
  flag fKey8, fKey4, fKey6, fKey2, fKeyU, fKeyD, fKeyShift, fRedraw;
...
LTimer:
    ...
    if (dr.fInside && dr.nInside == nInsideVeryFree &&
