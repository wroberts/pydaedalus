/*
** Daedalus (Version 3.2) File: draw.cpp
...
void WriteLineLevel(COOR *coor, CONST CMon &b, int height)
{
...
flag CreateOverviewLine(CONST CMon &b, COOR *coor)
{
  int x, y, x1, y1, x2, y2, count;
  ...
  WriteLineLevel(coor, b, ds.depmin);
  if (ds.depsiz == 0)
    return fTrue;
  WriteLineLevel(coor, b, ds.depmin + ds.depsiz);

  // Generate lines through the z-axis from the top to bottom of blocks.
  for (y = 0; y <= b.m_y; y++)
    for (x = 0; x <= b.m_x; x++) {
      count = b.Get(x - 1, y - 1) + b.Get(x, y - 1) +
        b.Get(x - 1, y) + b.Get(x, y);
      if (FOdd(count) ||
        (count == 2 && b.Get(x - 1, y - 1) == b.Get(x, y)) ||
        (!ds.fMerge && count > 0)) {
        x1 = ds.hormin + ds.horsiz*x;
        y1 = ds.vermin + ds.versiz*y;
        x2 = x1;
...
flag CreateCubicLine(CONST CMon3 &t, COOR *coor)
{
  int x, y, z, x1, y1, z1, count;
  flag fTest;
  ...
  for (y = 0; y <= t.m_x3; y++)
    for (z = 0; z <= t.m_z3; z++)
      for (x = 0; x <= t.m_y3; x++) {
        ...
        count = t.Get3(x - 1, y - 1, z) + t.Get3(x, y - 1, z) +
          t.Get3(x - 1, y, z) + t.Get3(x, y, z);
        fTest = t.Get3(x - 1, y - 1, z) == t.Get3(x, y, z);
        if (FOdd(count) || (count == 2 && fTest) || (!ds.fMerge && count > 0))
          WriteCoordinates(coor, x1, y1, z1, x1, y1, z1 - ds.depsiz);
        count = t.Get3(x - 1, y, z - 1) + t.Get3(x, y, z - 1) +
          t.Get3(x - 1, y, z) + t.Get3(x, y, z);
        fTest = t.Get3(x - 1, y, z - 1) == t.Get3(x, y, z);
        if (FOdd(count) || (count == 2 && fTest) || (!ds.fMerge && count > 0))
          WriteCoordinates(coor, x1, y1, z1, x1, y1 - ds.versiz, z1);
        count = t.Get3(x, y - 1, z - 1) + t.Get3(x, y, z - 1) +
          t.Get3(x, y - 1, z) + t.Get3(x, y, z);
        fTest = t.Get3(x, y - 1, z - 1) == t.Get3(x, y, z);
        if (FOdd(count) || (count == 2 && fTest) || (!ds.fMerge && count > 0))
...
long CreateWireframe(CONST CMon3 &b, COOR **pcoor, flag f3D)
{
  COOR *coor;
  ...
  ds.cCoorPatch = 0;
  if (!f3D)
    CreateOverviewLine(b, NULL);
  else
    CreateCubicLine(b, NULL);
  ...
  *pcoor = coor;
  ...
  ds.cCoorPatch = 0;
  if (!f3D)
    CreateOverviewLine(b, coor);
  else
    CreateCubicLine(b, coor);
...
#define KvObjectColor(c, x, y) \
  ((c) != NULL && (c)->FLegal(x, y) ? (c)->Get(x, y) : ds.kvObject)
#define KvObjectColor3(c, x, y, z, t) \
  KvObjectColor(c, (t).X2(x, z), (t).Y2(y, z))
  ...
  for (y = 0; y <= b.m_y; y++)
    for (x = 0; x <= b.m_x; x++) {
      ...
      if (b.Get(x, y) != b.Get(x, y-1) && x1 != x2) {
        kv = KvObjectColor(c, x, y - b.Get(x, y-1));
        pat[0].x = pat[1].x = x1;
        pat[2].x = pat[3].x = x2;
      ...
      if (b.Get(x, y) != b.Get(x-1, y) && y1 != y2) {
        kv = KvObjectColor(c, x - b.Get(x-1, y), y);
    ...
    for (z = 0; z <= t.m_z3; z++) {
      ...
      for (x = 0; x <= t.m_x3; x++) {
        ...
        if (t.Get3(x, y, z) != t.Get3(x, y, z-1)) {
          kv = KvObjectColor3(c, x, y, z - t.Get3(x, y, z-1), t);
          for (i = 0; i < 4; i++) {
        ...
        if (t.Get3(x, y, z) != t.Get3(x-1, y, z)) {
          kv = KvObjectColor3(c, x - t.Get3(x-1, y, z), y, z, t);
          for (i = 0; i < 4; i++) {
            pat[i].x = x1; pat[i].y = y1; pat[i].z = z1;
          }
          pat[2].y = pat[3].y = y2;
        ...
        if (t.Get3(x, y, z) != t.Get3(x, y-1, z)) {
          kv = KvObjectColor3(c, x, y - t.Get3(x, y-1, z), z, t);
          for (i = 0; i < 4; i++) {
            pat[i].x = x1; pat[i].y = y1; pat[i].z = z1;
          }
          pat[2].x = pat[3].x = x2;
