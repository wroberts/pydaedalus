/*
** Daedalus (Version 3.2) File: command.cpp
...
{cmdRenderAerial,          "Aerial",          "",     R2 | HG | B2 | M2},
...
{varInfFile,    "fSaveInfiniteToFile",     0},
...
void DoSetVariable(int ivar, CONST char *sz, int cch, long l)
{
  ...
  case varInfFile:    ms.fInfFile       = f; break;
  case varInfFileDos: ms.fInfFileDos    = f; break;
  ...
  case varStar:       dr.fStar         = f; break;
  case varStarCount:  dr.cStar         = n; break;
  ...
  case varStarColor:     dr.lStarColor    = n; break;
  case varSkyAll:        dr.fSkyAll       = f; break;
...
void GetVariable(int ivar, char **psz, int *pcch, long *pl)
{
  ...
  case varInfFile:    n = ms.fInfFile;       break;
  case varInfFileDos: n = ms.fInfFileDos;    break;
  ...
  case varStar:       n = dr.fStar;         break;
  case varStarCount:  n = dr.cStar;         break;
  ...
  case varStarColor:     n = dr.lStarColor;    break;
  case varSkyAll:        n = dr.fSkyAll;       break;
...
long EvalFunction(int ifun, char *rgsz[], CONST int *rgcch, CONST long *rgl)
{
  ...
  case funVer:    n = 3200;                           break;  // Daedalus 3.2
  ...
  if ((ch1 == '@' || ch1 == '$') && !fQuote) {
    ...
    if (ch1 == '@')
      *pl = LVar(ivar);
      ...
      if (ivar >= 0 && ivar < ws.cszVar && ws.rgszVar[ivar] != NULL) {
        *psz = ws.rgszVar[ivar];
...
LBitmapError:
  ...
  case cmdAllRandom:
    if (!bm.fColor) {
      if (FBetween(ms.nRndBias, -50, 50))
        ...
        bm.b.BitmapRandom(ms.nRndBias < 0 ? 1 : ms.nRndBias-1, NAbs(ms.nRndBias));
    } else {
  ...
  case cmdCreateForest:
    bm.b.CreateMazeForest(ms.fTreeWall);
    break;
  ...
  case cmdCreateRandom:
    ...
    case 2: bm.b.CreateMazePrim2();     break;
    ...
    case 7: bm.b.CreateMazeForest(fFalse); break;
  ...
  case cmdRenderAerial:
    FRenderAerial(bm.b, bm.coor, bm.ccoor);
    ds.fDidPatch = fFalse;
    break;
...
int RunCommandLine(char *szLine, FILE *file)
{
  ...
  if (ws.nMacroDepth > 100) {
    PrintSz_E("Command lines too heavily nested!");
    goto LError;
  }
LRestart:
  pchCur = szLine;
