/*
** Daedalus (Version 3.2) File: create2.cpp
  ...
  int xp, int yp, int nRndSeed, long *pxEntrance, long *pxExit)
{
  ...
  int xpMax = 1, ypMax = 1, xs, ys, i, nT;
  flag fX, fY;
  ...
  *pxEntrance = xEntrance; *pxExit = xExit;
  ...
  if (x0 >= 0 && x0 < xsMax && y0 >= 0 && y0 < ysMax) {
    if (ms.nFractalT <= vmtNested) {
      ...
      InitRndRgl(rgl, 4);
      nT = ms.nEntrancePos; ms.nEntrancePos = epCorner;
      bT.CreateMazeBinary();
      ms.nEntrancePos = nT;
      bT.Set1(1, 0);
      xT = (x0 > 0) << 1; yT = (y0 > 0) << 1;
  ...
  long *pxEntrance, long *pxExit)
{
  ...
  // Recursive division and Labyrinth virtual Mazes can be made all at once.
    ...
    Assert(xT == yT);
    i = (xT >> 3) - 1;
    LabyrinthPartial(i, (x0 - 1)*xpMax, (y0 - 1)*ypMax);
  ...
  int nRndSeed, long *pxEntrance, long *pxExit)
{
  ...
  rect.x1 = 0; rect.y1 = 0; rect.x2 = xs; rect.y2 = ys;
  pr = rgrect;
  *pr = rect;
  BitmapOff();
...
void CMaz::FractalGenerate3D(CMaz &bT, int w, int xp, int yp, int zp)
{
  ...
  // Render the Maze at appropriate zoom level and location.
  BlockMoveMaze3(bT, 0, 0, 0, bT.m_x3-2, bT.m_y3-2, bT.m_z3-1,
...
flag CMaz::FractalPartial3D(long x0, long y0, long z0, int z, int nRndSeed,
  long *pxEntrance, long *pxExit)
{
  char sz[cchSzMax];
  int xFractal = ms.xFractal, yFractal = ms.xFractal, zFractal = ms.yFractal,
    xpMax = 1, ypMax = 1, zpMax = 1, i;
  long xT, yT, zT;
  ...
  // Create the bitmap for the fractal Maze.
...
flag CMaz::CreateMazePlanair()
{
  ...
  if (!FBitmapSizeSet(m_x3 * Min(jz, jw), m_y3 * ((jz + jw-1) / jw)))
    return fFalse;
  BitmapOff();
  if (!FEnsureMazeSize(3, femsNoResize))
    return fFalse;
...
FILE *FileOpenInfinite(CONST char *szType)
{
  char sz[cchSzDef];
  FILE *file;

  // If writing to multiple files, include numbers in the filename.
  if (ms.nInfFileCutoff > 0)
    sprintf(S(sz), "inf%05d.txt", ms.nInfGen / ms.nInfFileCutoff);
  else
    sprintf(S(sz), "infinite.txt");
  file = FileOpen(sz, szType);
  return file;
}
...
flag CMaz::CreateMazeInfinite(int phase)
{
  ...
  if (phase == infStart) {
    ...
    if (ms.fInfFile) {
      if (ms.nInfFileCutoff > 0 && ms.nInfGen % ms.nInfFileCutoff == 0) {
        if (ms.fileInf != NULL)
          fclose(ms.fileInf);
        ms.fileInf = FileOpenInfinite("w");
        if (ms.fileInf == NULL)
  ...
  loop {
    if (!Get(x, y)) {
      if (ms.fRiver || fHunt) {
        d = DirFindUncreatedCore(&x, &y, fFalse);
        fHunt = d < 0;
      } else {
        d = RndDir();
        xnew = x + xoff2[d];
